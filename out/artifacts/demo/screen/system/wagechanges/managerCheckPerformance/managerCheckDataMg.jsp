...
$(document).ready(function() {
			...
			/*查询表格初始化  设置默认查询条件*/
			var fData={'actionId':'doBiz','start':'0','limit':'10','txcode':'managerCheckDataMg'};
...
//保存
function doSave(){
	var extParam={'option':actionType,'txcode':'managerCheckDataMg','actionId':'doBiz'};
...
function del(){
	...
	var param={'option':"remove",'txcode':"managerCheckDataMg",'userids': iwapGrid.getCheckValues(),'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
...
function delOne(obj){
	...
	var param={'option':"remove",'txcode':"managerCheckDataMg",'userids': iwapGrid.getCurrentRow()['D_ID'],'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
		 	return alert("删除失败:"+rs['header']['msg']);
		 }else{
