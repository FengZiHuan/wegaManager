...
<body class="iwapui center_body">
	...
	<div class="table_box">
		...
		<form method="post" action="iwap.ctrl" id="dialogarea2">
			<input type="hidden" id="txcode" name="txcode" size="15" value="mergeSummaryCheck">
        	<input type="hidden" id="actionId" name="actionId" size="15" value="doBiz">
        	<input type="hidden" id="option" name="option" size="15" value="viewDetail">
        	<input type="hidden" id="month" name="month" size="15"/>
        	<input type="hidden" id="module" name="module" size="15" value="giveBack"/>
		<table id="iwapGrid"
			...
			<tr>
				...
				<th data-grid-name="BRANCH_TOTAL" class="tl">汇总分行数量</th>
				<th data-grid-name="TOTAL_SUM" class="tl">总金额</th>
				<th data-grid-name="ROLE_ID" option="option" option-html=''><span>操作</span>
					<s>
					    <a href="javaScript:void(0)" class="editId" onclick="viewDetail(this)" id ="viewDetails">查看</a>&nbsp;|&nbsp;
					    <a href="javaScript:void(0)" class="editId" onclick="edit(this)" id ="editOne">审核</a>&nbsp;|&nbsp;
...
$(document).ready(function() {
			...
			/*查询表格初始化  设置默认查询条件*/
			var fData={'actionId':'doBiz','start':'0','limit':'10','txcode':'mergeSummaryCheck'};
...
//退回
function giveBack(obj){
	if (!confirm("确定要退回吗?请确定!")){
		return;
	}
	var param={'option':"giveBack",'txcode':"mergeSummaryCheck",'payMonth': iwapGrid.getCurrentRow()['PAY_MONTH'],'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
		 	return alert("退回失败:"+rs['header']['msg']);
		 }else{
		 	alert("退回成功");
...
function exportData(){
	...
	var data = {'exportFlag':'1','filetype':'xlsx','txcode':'fiveRisksOneGoldCheck','payMonth':payMonth,'branchNames':branchNames,'actionId':'doBiz','start':'0','limit':'1000'};
	var param="";
	for (var key in data) {
		param += key + "=" + data[key] + "&";
	}
