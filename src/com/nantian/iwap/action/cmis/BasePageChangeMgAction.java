...
public class BasePageChangeMgAction extends TransactionBizAction {
    private Map config;
    ...
    private static Logger log = Logger.getLogger(BasePageChangeMgAction.class);
    ...
    private int export(DTBHelper dtbHelper) {
        ...
        String sqlStr="SELECT B_CMIS,B_NAME,B_POST,B_POSTTYPE,B_POSTTIMES,B_TP,B_TPTIMES,B_TGW,B_TGWTIMES,B_EDU," +
                "B_EDUTIMES,B_WORKYEAR,B_WD,B_HARD,B_SPE,B_TIMES,B_REASON,B_REMARK,B_STATUS,B_IDCARD,B_SENDDEPT FROM  PEOPLE_BASEPAY_CHANGES";
        ...
        try {
            ...
            if (userids.equals("") || userids.contains("{{value}}")) {
                ...
                for (String user : userarr) {
                    ...
                    sqlStr="SELECT B_CMIS,B_NAME,B_POST,B_POSTTYPE,B_POSTTIMES,B_TP,B_TPTIMES,B_TGW,B_TGWTIMES,B_EDU," +
                            "B_EDUTIMES,B_WORKYEAR,B_WD,B_HARD,B_SPE,B_TIMES,B_REASON,B_REMARK,B_STATUS,B_IDCARD,B_SENDDEPT FROM  PEOPLE_BASEPAY_CHANGES where b_id=? ";
            ...
            String[] t = new String[]{"B_CMIS", "B_NAME","B_POST", "B_POSTTYPE", "B_POSTTIMES", "B_TP", "B_TPTIMES"
                    , "B_TGW", "B_TGWTIMES", "B_EDU", "B_EDUTIMES", "B_WORKYEAR","B_WD","B_HARD","B_SPE","B_TIMES","B_REASON","B_REMARK","B_STATUS","B_IDCARD","B_SENDDEPT"};
            for (int i = 0; i < t.length; i++) {
                titleId.add(t[i]);
            }
            String s = "overTime";

            String[] head0 = new String[]{"员工号","姓名","行政职务", "行政职级", "行政职务（职级）聘任时间", "专业技术职务", "专业技术职务聘任时间", "工人技术等级", "工人技术等级聘任时间", "最高学历"
                    , "最高学历取得时间", "工龄","营业网点员工","艰边津贴","西藏特殊津贴","变动时间","变动原因","备注", "执行状态",  "身份证号", "发薪机构"};

            String[] head1 = new String[]{"员工号","姓名","行政职务", "行政职级", "行政职务（职级）聘任时间", "专业技术职务", "专业技术职务聘任时间", "工人技术等级", "工人技术等级聘任时间", "最高学历"
                    , "最高学历取得时间", "工龄","营业网点员工","艰边津贴","西藏特殊津贴","变动时间","变动原因","备注", "执行状态",  "身份证号", "发薪机构"};
            String[] headnum0 = new String[]{"0,1,0,0", "0,1,1,1", "0,1,2,2", "0,1,3,3", "0,1,4,4", "0,1,5,5", "0,1,6,6", "0,1,7,7",
                    "0,1,8,8", "0,1,9,9", "0,1,10,10","0,1,11,11","0,1,12,12","0,1,13,13","0,1,14,14","0,1,15,15","0,1,16,16","0,1,17,17","0,1,18,18","0,1,19,19","0,1,20,20","0,1,21,21",};
    ...
    protected int query(DTBHelper dtbHelper) throws BizActionException {
        ...
        try {
            ...
            String sqlStr="SELECT B_ID,B_CMIS,B_NAME,B_POST,B_POSTTYPE,B_POSTTIMES,B_TP,B_TPTIMES,B_TGW,B_TGWTIMES,B_EDU," +
                    "B_EDUTIMES,B_WORKYEAR,B_WD,B_HARD,B_SPE,B_TIMES,B_REASON,B_REMARK,B_STATUS,B_IDCARD,B_SENDDEPT FROM  PEOPLE_BASEPAY_CHANGES where 1=1 ";

            if (!"".equals(name)) {
                sqlStr += " and b_name like '%" + name + "%'";
            }
            if (!"".equals(fmis)) {
                sqlStr += " and b_cmis = '" +fmis + "'";
            }
