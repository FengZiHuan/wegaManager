...
public class DeptUpdateMgAction extends TransactionBizAction {
    private Map config;
    ...
    protected int query(DTBHelper dtbHelper) throws BizActionException {
        ...
        try {
            ...
            String sqlStr = "select * from nh_dept where 1=1 ";
            String d_name = dtbHelper.getStringValue("name");
            String d_no = dtbHelper.getStringValue("no");
            if (!"".equals(d_name)) {
                sqlStr += " and d_name like '%" + d_name + "%'";
            }
            if (!"".equals(d_no)) {
                sqlStr += " and d_no = '" + d_no + "'";
            }
