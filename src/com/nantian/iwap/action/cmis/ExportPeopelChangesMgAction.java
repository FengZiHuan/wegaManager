...
public class ExportPeopelChangesMgAction extends TransactionBizAction {
    private Map config;
    ...
    private static Logger log = Logger.getLogger(ExportPeopelChangesMgAction.class);
    ...
    private int export(DTBHelper dtbHelper) {
        ...
        String sqlStr="SELECT to_char(P_STARTTIMES,'yyyy-MM-dd') P_STARTTIMES,to_char(P_ENDTIMES,'yyyy-MM-dd') P_ENDTIMES ,P_NAME,P_CMIS,P_PEOPLETYPE,P_FMIS,P_FMISC,P_MODEL,P_CSB,P_CMISDEPT,P_IDCARD,P_SENDDEPT,P_CMISDEPTNO,P_STATUS FROM NH_PEOPLECHANGES";
        ...
        try {
            ...
            if (userids.equals("") || userids.contains("{{value}}")) {
                ...
                for (String user : userarr) {
                    ...
                    sqlStr = "SELECT to_char(P_STARTTIMES,'yyyy-MM-dd') P_STARTTIMES,to_char(P_ENDTIMES,'yyyy-MM-dd')" +
                            " P_ENDTIMES , P_NAME,P_CMIS,P_PEOPLETYPE,P_FMIS,P_FMISC,P_MODEL,P_CSB,P_CMISDEPT,P_IDCARD,P_SENDDEPT,P_CMISDEPTNO,P_STATUS FROM NH_PEOPLECHANGES where p_id=?";
            ...
            String[] t = new String[]{"P_CMIS", "P_NAME","P_CMISDEPTNO", "P_PEOPLETYPE", "P_FMIS", "P_FMISC", "P_MODEL"
                    , "P_CSB", "P_STARTTIMES", "P_ENDTIMES", "P_STATUS", "P_CMISDEPT","P_IDCARD","P_SENDDEPT"};
            for (int i = 0; i < t.length; i++) {
                titleId.add(t[i]);
            }
            String s = "overTime";
            String[] head0 = new String[]{"员工号","姓名","发薪部门号", "薪酬人员类别", "FMIS员工号", "fmis明细核算中心号", "适用薪酬模板", "适用参数表", "起薪时间", "停薪时间"
                    , "发薪计算标志", "发薪部门", "身份证号", "发薪机构"};
            String[] head1 = new String[]{"员工号","姓名","发薪部门号", "薪酬人员类别", "FMIS员工号", "fmis明细核算中心号", "适用薪酬模板", "适用参数表", "起薪时间", "停薪时间"
                    , "发薪计算标志", "发薪部门", "身份证号", "发薪机构"};;
            String[] headnum0 = new String[]{"0,1,0,0", "0,1,1,1", "0,1,2,2", "0,1,3,3", "0,1,4,4", "0,1,5,5", "0,1,6,6", "0,1,7,7",
                    "0,1,8,8", "0,1,9,9", "0,1,10,10","0,1,11,11","0,1,12,12","0,1,13,13","0,1,14,14",};
    ...
    protected int query(DTBHelper dtbHelper) throws BizActionException {
        ...
        try {
            ...
            String sqlStr="SELECT to_char(P_STARTTIMES,'yyyy-MM-dd') P_STARTTIMES,to_char(P_ENDTIMES,'yyyy-MM-dd') P_ENDTIMES , P_ID,P_NAME,P_CMIS,P_PEOPLETYPE,P_FMIS,P_FMISC,P_MODEL,P_CSB,P_CMISDEPT,P_IDCARD,P_SENDDEPT,P_CMISDEPTNO,P_STATUS FROM NH_PEOPLECHANGES";

            if (!"".equals(name)) {
                sqlStr += " and p_name like '%" + name + "%'";
            }
            if (!"".equals(fmis)) {
                sqlStr += " and p_fmis = '" +fmis + "'";
            }
