...
public class IsuranseChangeMgAction extends TransactionBizAction {
    private Map config;
    ...
    private static Logger log = Logger.getLogger(IsuranseChangeMgAction.class);
    ...
    private int export(DTBHelper dtbHelper) {
        ...
        String sqlStr="SELECT I_CMIS,I_NAME,I_SOCIAL_SECURITY,I_PUBILC_FUNDS,I_OLDBXWEGAS,I_YLBXWEGAS," +
                "I_SYBXWEGAS,I_GSBXWEGAS,I_SYUBXWEGAS,I_QNWEGAS,I_GZJWEGAS,I_BGZJWEGAS,I_ZFBTWEGAS,I_TIMES,I_STATUS,I_SENDDEPT,I_IDCARD,I_REMARK FROM PEOPLE_INSURANSE_CHANGES";
        ...
        try {
            ...
            if (userids.equals("")) {
                ...
                for (String user : userarr) {
                   ...
                   sqlStr="SELECT I_CMIS,I_NAME,I_SOCIAL_SECURITY,I_PUBILC_FUNDS,I_OLDBXWEGAS,I_YLBXWEGAS," +
                            "I_SYBXWEGAS,I_GSBXWEGAS,I_SYUBXWEGAS,I_QNWEGAS,I_GZJWEGAS,I_BGZJWEGAS,I_ZFBTWEGAS,I_TIMES,I_STATUS,I_SENDDEPT,I_IDCARD,I_REMARK FROM PEOPLE_INSURANSE_CHANGES where I_ID=?";
            ...
            String[] t = new String[]{"I_CMIS", "I_NAME","I_SOCIAL_SECURITY", "I_PUBILC_FUNDS", "I_OLDBXWEGAS", "I_YLBXWEGAS", "I_SYBXWEGAS"
                    , "I_GSBXWEGAS", "I_SYUBXWEGAS", "I_QNWEGAS", "I_GZJWEGAS", "I_BGZJWEGAS","I_ZFBTWEGAS","I_TIMES"
                     ,"I_REMARK","I_STATUS","I_IDCARD","I_SENDDEPT"};
            for (int i = 0; i < t.length; i++) {
                titleId.add(t[i]);
            }
            String s = "overTime";

            String[] head0 = new String[]{"员工号","姓名","社保号", "公积金账号", "养老保险缴费基数", "医疗保险缴费基数", "失业保险缴费基数", "工伤保险基数", "生育保险基数", "企业年金基数"
                    , "公积金缴费基数", "补充公积金缴费基数", "住房补贴基数", "变动时间", "备注", "执行状态", "身份证号", "发薪机构"};
            String[] head1 = new String[]{"员工号","姓名","社保号", "公积金账号", "养老保险缴费基数", "医疗保险缴费基数", "失业保险缴费基数", "工伤保险基数", "生育保险基数", "企业年金基数"
                    , "公积金缴费基数", "补充公积金缴费基数", "住房补贴基数", "变动时间", "备注", "执行状态", "身份证号", "发薪机构"};
            String[] headnum0 = new String[]{"0,1,0,0", "0,1,1,1", "0,1,2,2", "0,1,3,3", "0,1,4,4", "0,1,5,5", "0,1,6,6", "0,1,7,7",
                    "0,1,8,8", "0,1,9,9", "0,1,10,10","0,1,11,11","0,1,12,12","0,1,13,13","0,1,14,14","0,1,15,15","0,1,16,16","0,1,17,17","0,1,18,18"};
    ...
    protected int query(DTBHelper dtbHelper) throws BizActionException {
        ...
        try {
            ...
            String sqlStr="SELECT I_ID,I_CMIS,I_NAME,I_SOCIAL_SECURITY,I_PUBILC_FUNDS,I_OLDBXWEGAS,I_YLBXWEGAS," +
                    "I_SYBXWEGAS,I_GSBXWEGAS,I_SYUBXWEGAS,I_QNWEGAS,I_GZJWEGAS,I_BGZJWEGAS,I_ZFBTWEGAS,I_TIMES,I_STATUS,I_SENDDEPT,I_IDCARD,I_REMARK FROM PEOPLE_INSURANSE_CHANGES where 1=1";


            if (!"".equals(name)) {
                sqlStr += " and i_name like '%" + name + "%'";
            }
            if (!"".equals(fmis)) {
                sqlStr += " and i_cmis = '" +fmis + "'";
            }
