...
public class FiveRisksOneGold extends TransactionBizAction{

	private static Logger log = Logger.getLogger(FiveRisksOneGold.class);
	...
	protected int add(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			personalCount=gjjper.add(ylbxper).add(sybxper).add(ylaobxper);
			...
			sbf.append("SYBX_PERSONAL_MONTHPAY, YLAOBX_COMPANY_MONTHPAY,YLAOBX_PERSONAL_MONTHPAY, GSBX_COMPANY_MONTHPAY, SYUBX_COMPANY_MONTHPAY,");
			sbf.append("COMPANY_COUNT, PERSONAL_COUNT,COMPANYANDPERSONAL_COUNT) ");
			...
			dbBean.executeUpdate(sbf.toString(), userName, fmisNo, branchName, payMonth, gjjmscompay, gjjjscompay,gjjmspersonal,gjjjspersonal,ylbxcom.doubleValue(),
					ylbxper.doubleValue(),sybxcom.doubleValue(),sybxper.doubleValue(),ylaobxcom.doubleValue(),ylaobxper.doubleValue(),gsbxcom.doubleValue(),
					syubxcom.doubleValue(),companyCount.doubleValue(),personalCount.doubleValue(),companyAndPersonalCount.doubleValue());
	...
	protected int save(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			dbBean.executeUpdate(sbf.toString(), userName, fmisNo, branchName, payMonth, gjjmscompay, gjjjscompay,gjjmspersonal,gjjjspersonal,ylbxcom.doubleValue(),
					ylbxper.doubleValue(),sybxcom.doubleValue(),sybxper.doubleValue(),ylaobxcom.doubleValue(),ylaobxper.doubleValue(),gsbxcom.doubleValue(),
					syubxcom.doubleValue(),companyCount.doubleValue(),personalCount.doubleValue(),companyAndPersonalCount.doubleValue(),fmisNo);
			dbBean.executeCommit();
			flag = 1;
		} catch (Exception e) {
	...
	protected int remove(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			for (String id : idarr) {
				...
				u_cnt++;
				int i = dbBean.executeUpdate("delete from fiveRisks_oneGold where ID = ?", id);
				if (i == 1) {
					s_cnt++;
				}
			}
